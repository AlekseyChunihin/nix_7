...
public class ConvertDatesToListOfStringFromNumbersUtil {

    public static ArrayList<String> getListOfStringFromNumbers(ArrayList<Pair> dates) {
        ArrayList<String> stringsOfNumbers = new ArrayList<>();
        for (int i = 0; i < dates.size(); i++) {
            if (dates.get(i).getDateFormat().equals(DateFormatConstants.FIRST_FORMAT)) {
                if (checkFirstFormat(dates.get(i).getDateValue())) {
                    stringsOfNumbers.add(convertToStringFromNumbers(dates.get(i).getDateValue()));
                }
            } else if (dates.get(i).getDateFormat().equals(DateFormatConstants.SECOND_FORMAT)) {
                if (checkSecondFormat(dates.get(i).getDateValue())) {
                    stringsOfNumbers.add(convertToStringFromNumbers(dates.get(i).getDateValue()));
                }
            } else if (dates.get(i).getDateFormat().equals(DateFormatConstants.THIRD_FORMAT)) {
