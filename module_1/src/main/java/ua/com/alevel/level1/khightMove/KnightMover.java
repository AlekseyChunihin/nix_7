...
public class KnightMover {
    ...
    public KnightMover(String name, String color) {
        this.name = name;
        this.color = color;
    }
    ...
    public int[][] getMove() {
        ...
        for (int runNum = 1; runNum <= 2; runNum++) {
            while (true) {
                if (runNum == 1) {
                    System.out.print(name + ", input location of a knight to make a move. (example: D2 or d2)\n>> ");
                } else {
                    System.out.print(name + ", input end location of a knight. (example: D4 or d4)\n>> ");
                }
                String moveIn = scanner.nextLine().trim();
                if (moveIn.equals("stop")) {
                    return null;
                }
